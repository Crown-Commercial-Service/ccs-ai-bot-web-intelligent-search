...
def invoke_llm_stage_1(user_query: str):
    ...
    2. 'recommendation': Requests for framework suggestions (e.g., "Which framework for office furniture?", "What agreement for temporary medical staff?")
...
def Startup_agenet(state: State):
    ...
    results = search_index(query, data_classification, os.getenv('azure_index_normalIndex'), os.getenv('azure_search_service_new_endpoint'))
...
def Framework_details_agent(state: State):
    ...
    results = search_index(query, data_classification, os.getenv('azure_index_normalIndex'), os.getenv('azure_search_service_new_endpoint'), Framework_classification, 5)
...
state = app.invoke({"query": "we are from the NHS trust. Which agreements would be suitable for hiring health workers and IT professionals?"})
print(state["output"])
print(set(state['framework_numbers']))
